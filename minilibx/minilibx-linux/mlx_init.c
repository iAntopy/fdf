...
void	*mlx_init()
{
	...
	printf("mlx_init : starting\n");
	write(1, "WOW", 3);
	if (!(xvar = malloc(sizeof(*xvar))))
		return ((void*)0);
	printf("mlx_init : x_var malloc SUCCESS\n");
	if ((xvar->display = XOpenDisplay("")) == 0)
	{
		free(xvar);
		return ((void*)0);
	}
	printf("mlx_init : XOpen_Display SUCCESS\n");
	...
	printf("mlx_init : in get visual SUCCESS\n");
	xvar->win_list = 0;
	xvar->loop_hook = 0;
	...
	xvar->end_loop = 0;
	printf("mlx_init : exits peacefully with ptr %p\n", xvar);
	return (xvar);
}
