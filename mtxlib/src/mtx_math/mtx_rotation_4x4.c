...
/*   Updated: 2022/11/28 05:30:26 by iamongeo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	__fill_YXZ_rotmat_4x4(const float s[3], const float c[3], const float *arr)
{
...
{
	static const float	sins[3] = {sinf(rll), sinf(pch), sinf(yaw)};
	static const float	coss[3] = {cosf(rll), cosf(pch), cosf(yaw)};
...
void	__mtx_rotation_matrix_4x4(float rll, float pch, float yaw, float *out)
{
	...
	__mtx_dotf_4x4_4x4(4, out, arr1, arr2);
	__fill_yaw_rotmat_4x4(sinf(yaw), cos(yaw), arr1);
	__mtx_dotf_4x4_4x4(4, arr2, arr1, out);
}
